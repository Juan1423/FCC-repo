        ...
        <Paper elevation={0} sx={{ border: '1px solid', borderColor: '#e7e5e4', borderRadius: 2, bgcolor: '#ffffff', overflow: 'hidden' }}>
          <Box sx={{ px: { xs: 2, sm: 3 }, py: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1.5, borderBottom: '1px solid', borderColor: '#e7e5e4', bgcolor: '#fafaf9' }}>
            ...
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
              <TextField size="small" placeholder="Buscar documentos..." value={search} onChange={(e) => setSearch(e.target.value)} aria-label="Buscar documentos por nombre, responsable, tipo o estado" slotProps={{ input: { startAdornment: <SearchIcon sx={{ color: '#a8a29e', mr: 0.5, fontSize: 18 }} /> } }} sx={{ width: { xs: 1, sm: 'auto' }, '& .MuiOutlinedInput-root': { borderRadius: '8px', bgcolor: '#ffffff', fontSize: '0.85rem', '& fieldset': { borderColor: '#e7e5e4' }, '&:hover fieldset': { borderColor: '#d6d3d1' }, '&.Mui-focused fieldset': { borderColor: '#0d9488' } } }} />
          ...
          {loading ? (
            ...
            <Table size="small" aria-label="Lista de documentos" stickyHeader sx={{ minWidth: 650, '& .MuiTableCell-root': { wordBreak: 'break-word', overflowWrap: 'break-word' } }}>
              <TableHead>
                <TableRow>
                  {['Nombre', 'Tipo', 'Estado', 'Versión', 'Acciones'].map((h) => (<TableCell key={h} sx={{ display: h === 'Versión' ? { xs: 'none', md: 'table-cell' } : undefined, fontWeight: 600, color: '#57534e', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '2px solid', borderColor: '#e7e5e4', py: 1.5 }} align={h === 'Acciones' ? 'right' : 'left'}>{h}</TableCell>))}
                </TableRow>
              </TableHead>
              <TableBody>
                {filtered.length === 0 ? (
                  ...
                  paginatedItems.map((row) => (
                    <TableRow key={row.id_documento} sx={{ '&:hover': { bgcolor: '#fafaf9' }, '&:focus-within': { bgcolor: '#f0fdfa' }, transition: 'background-color 0.15s ease' }}>
                      ...
                      <TableCell sx={{ display: { xs: 'none', md: 'table-cell' }, color: '#57534e', fontSize: '0.85rem' }}>{row.version_documento || '—'}</TableCell>
