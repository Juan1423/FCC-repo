        ...
        <TableContainer component={Paper} elevation={0} sx={{ border: '1px solid', borderColor: '#e7e5e4', borderRadius: 2, overflow: { xs: 'auto', sm: 'visible' }, bgcolor: '#ffffff', '&::before': { content: '""', position: 'absolute', top: 0, left: 0, right: 0, height: 3, bgcolor: '#0d9488', borderTopLeftRadius: 2, borderTopRightRadius: 2, zIndex: 1 } }}>
          <Box sx={{ px: { xs: 2, sm: 3 }, py: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1.5, borderBottom: '1px solid', borderColor: '#e7e5e4', bgcolor: '#fafaf9' }}>
            ...
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
              <TextField size="small" placeholder="Buscar indicadores..." value={search} onChange={(e) => setSearch(e.target.value)} aria-label="Buscar indicadores por nombre, tipo o estado" slotProps={{ input: { startAdornment: <SearchIcon sx={{ color: '#a8a29e', mr: 0.5, fontSize: 18 }} /> } }} sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px', bgcolor: '#ffffff', fontSize: '0.85rem', '& fieldset': { borderColor: '#e7e5e4' }, '&:hover fieldset': { borderColor: '#d6d3d1' }, '&.Mui-focused fieldset': { borderColor: '#0d9488' } } }} />
          ...
          {loading ? (
            ...
            <Table size="small" aria-label="Lista de indicadores">
              <TableHead>
                <TableRow>
                  {['Nombre', 'Tipo', 'Estado', 'Valor', 'Acciones'].map((h) => (<TableCell key={h} sx={{ fontWeight: 600, color: '#57534e', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '2px solid', borderColor: '#e7e5e4', py: 1.5 }} align={h === 'Acciones' ? 'right' : 'left'}>{h}</TableCell>))}
                </TableRow>
              </TableHead>
              <TableBody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((row) => (
                    <TableRow key={row.id_indicador} sx={{ '&:hover': { bgcolor: '#fafaf9' }, '&:focus-within': { bgcolor: '#f0fdfa' }, transition: 'background-color 0.15s ease' }}>
                      ...
                      <TableCell sx={{ color: '#57534e', fontSize: '0.85rem' }}>{row.valor_indicador != null ? row.valor_indicador : '—'}</TableCell>
