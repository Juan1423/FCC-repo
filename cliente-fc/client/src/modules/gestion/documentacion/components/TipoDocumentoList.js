    ...
    <>
      <TableContainer component={Paper} elevation={0} sx={{ border: 'none', borderRadius: 0 }}>
        <Box sx={{ p: { xs: 1.5, sm: 2 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1, borderBottom: '1px solid', borderColor: '#e7e5e4', bgcolor: '#fafaf9' }}>
          <Typography variant="h6" component="h2" sx={{ fontWeight: 600, fontSize: '1rem', color: '#1c1917' }}>Tipos de documento</Typography>
          <Button variant="contained" size="small" onClick={openCreate} aria-label="Crear nuevo tipo de documento" sx={{ bgcolor: '#0d9488', '&:hover': { bgcolor: '#0f766e' }, textTransform: 'none', fontWeight: 600, borderRadius: 1.5, px: 2 }}>Nuevo tipo</Button>
        </Box>
        {loading ? (
          <Box display="flex" justifyContent="center" p={3} role="status" aria-live="polite" aria-label="Cargando tipos de documento">
          ...
          <Table size="small" aria-label="Lista de tipos de documento">
            ...
            <TableBody>
              {items.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} sx={{ textAlign: 'center', color: '#a8a29e', py: 4 }}>No hay tipos de documento registrados.</TableCell>
                </TableRow>
              ) : (
                items.map((row) => (
                  <TableRow key={row.id_tipo_documento} sx={{ '&:hover': { bgcolor: '#fafaf9' } }}>
                    <TableCell sx={{ fontWeight: 500, color: '#1c1917' }}>{row.nombre_tipo_documento}</TableCell>
                    <TableCell sx={{ color: '#57534e' }}>{row.descripcion_tipo_documento}</TableCell>
                    <TableCell align="right" sx={{ whiteSpace: 'nowrap' }}>
                      <Button size="small" onClick={() => openEdit(row)} sx={{ mr: 0.5, textTransform: 'none', fontWeight: 500, color: '#0d9488', minWidth: 0 }} aria-label={`Editar ${row.nombre_tipo_documento}`}>Editar</Button>
                      <Button size="small" color="error" onClick={() => handleDelete(row.id_tipo_documento, row.nombre_tipo_documento)} sx={{ textTransform: 'none', fontWeight: 500, minWidth: 0 }} aria-label={`Eliminar ${row.nombre_tipo_documento}`}>Eliminar</Button>
                    </TableCell>
                  </TableRow>
                ))
              )}
      ...
      <Dialog open={openModal} onClose={() => setOpenModal(false)} maxWidth="sm" fullWidth aria-labelledby={dialogTitleId}>
        <DialogTitle id={dialogTitleId} sx={{ fontWeight: 700, color: '#1c1917' }}>{editingId ? 'Editar tipo de documento' : 'Nuevo tipo de documento'}</DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <TextField fullWidth label="Nombre del tipo" value={form.nombre_tipo_documento} onChange={(e) => setForm({ ...form, nombre_tipo_documento: e.target.value })} sx={{ mb: 2 }} />
