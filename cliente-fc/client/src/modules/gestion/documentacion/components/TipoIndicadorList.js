...
const emptyForm = () => ({ nombre_tipo_indicador: "", descripcion_tipo_indicador: "" });
...
const TipoIndicadorList = () => {
  ...
  const liveId = "tipo-indicador-live-announce";
  ...
  useEffect(() => {
    ...
    setFiltered(items.filter((t) =>
      (t.nombre_tipo_indicador && t.nombre_tipo_indicador.toLowerCase().includes(q)) ||
      (t.descripcion_tipo_indicador && t.descripcion_tipo_indicador.toLowerCase().includes(q))
    ));
  }, [search, items]);
  ...
  const openEdit = (row) => {
    ...
    setForm({ nombre_tipo_indicador: row.nombre_tipo_indicador || "", descripcion_tipo_indicador: row.descripcion_tipo_indicador || "" });
    setOpenModal(true);
  };

  const handleSave = async () => {
    try {
      if (editingId) await documentacionService.updateTipoIndicador(editingId, form);
      else await documentacionService.createTipoIndicador(form);
      setOpenModal(false);
      announce(editingId ? "Tipo de indicador actualizado" : "Tipo de indicador creado");
      await load();
      setError(null);
    } catch (e) {
  ...
  const handleDelete = async (id, nombre) => {
    ...
    try {
      ...
      announce(`Tipo de indicador "${nombre}" eliminado`);
      await load();
    } catch (e) {
    ...
    <>
      <Box sx={{ opacity: animating ? 0 : 1, transform: animating ? "translateY(8px)" : "translateY(0)", transition: "opacity 0.5s ease, transform 0.5s ease" }}>
        <Paper elevation={0} sx={{ border: "1px solid", borderColor: "#e7e5e4", borderRadius: 2, bgcolor: "#ffffff", overflow: "hidden" }}>
          <Box sx={{ px: { xs: 2, sm: 3 }, py: 2, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 1.5, borderBottom: "1px solid", borderColor: "#e7e5e4", bgcolor: "#fafaf9" }}>
            <Box>
              <Typography variant="h6" component="h2" sx={{ fontWeight: 700, fontSize: "1.05rem", color: "#1c1917", letterSpacing: "-0.01em" }}>Tipos de Indicador</Typography>
          ...
          {loading ? (
            <Box display="flex" justifyContent="center" p={4} role="status" aria-live="polite" aria-label="Cargando tipos de indicador"><CircularProgress size={28} aria-hidden="true" sx={{ color: "#0d9488" }} /><Typography variant="srOnly" sx={{ position: "absolute", width: 1, height: 1, overflow: "hidden", clip: "rect(0,0,0,0)" }}>Cargando tipos de indicador...</Typography></Box>
          ) : error ? (
            ...
            <Table size="small" aria-label="Lista de tipos de indicador" stickyHeader sx={{ minWidth: 650, "& .MuiTableCell-root": { wordBreak: "break-word", overflowWrap: "break-word" } }}>
              <TableHead>
                <TableRow>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  <TableRow><TableCell colSpan={3} sx={{ textAlign: "center", py: 5 }}>
                    ...
                    <Typography sx={{ color: "#a8a29e", fontWeight: 500 }}>{search ? "No se encontraron tipos con ese criterio." : "No hay tipos de indicador registrados."}</Typography>
                    {!search && <Button size="small" onClick={openCreate} startIcon={<AddIcon />} sx={{ mt: 1.5, textTransform: "none", fontWeight: 600, color: "#0d9488", borderRadius: "8px" }} aria-label="Crear el primer tipo de indicador">Crear primer tipo</Button>}
                  </TableCell></TableRow>
                ) : (
                  paginatedItems.map((row) => (
                    <TableRow key={row.id_tipo_indicador} sx={{ "&:hover": { bgcolor: "#fafaf9" }, "&:focus-within": { bgcolor: "#f0fdfa" }, transition: "background-color 0.15s ease" }}>
                      <TableCell sx={{ fontWeight: 600, color: "#1c1917", fontSize: "0.85rem" }}>
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}><BarChartIcon sx={{ color: "#0d9488", fontSize: 18, opacity: 0.6 }} />{row.nombre_tipo_indicador}</Box>
                      </TableCell>
                      <TableCell sx={{ color: "#57534e", fontSize: "0.85rem", maxWidth: 360 }}>{row.descripcion_tipo_indicador || "—"}</TableCell>
                      <TableCell align="right" sx={{ whiteSpace: "nowrap" }}>
                        <Tooltip title="Editar tipo de indicador" arrow><IconButton size="small" onClick={() => openEdit(row)} aria-label={`Editar ${row.nombre_tipo_indicador}`} sx={{ color: "#0d9488", borderRadius: "8px", mr: 0.5, "&:hover": { bgcolor: "rgba(13,148,136,0.08)" } }}><EditIcon fontSize="small" /></IconButton></Tooltip>
                        <Tooltip title="Eliminar tipo de indicador" arrow><IconButton size="small" onClick={() => handleDelete(row.id_tipo_indicador, row.nombre_tipo_indicador)} aria-label={`Eliminar ${row.nombre_tipo_indicador}`} sx={{ color: "#ef4444", borderRadius: "8px", "&:hover": { bgcolor: "rgba(239,68,68,0.08)" } }}><DeleteIcon fontSize="small" /></IconButton></Tooltip>
                      </TableCell>
                    </TableRow>
                  ))
                )}
      ...
      <Dialog open={openModal} onClose={() => setOpenModal(false)} maxWidth="sm" fullWidth aria-labelledby={dialogTitleId}
        ...
        <DialogTitle id={dialogTitleId} sx={{ fontWeight: 700, color: "#1c1917", fontSize: "1.1rem", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid", borderColor: "#e7e5e4", px: 3, py: 2 }}>
          {editingId ? "Editar tipo de indicador" : "Nuevo tipo de indicador"}
        ...
        <DialogContent sx={{ pt: 2.5, px: 3 }}>
          <TextField fullWidth label="Nombre del tipo" value={form.nombre_tipo_indicador} onChange={(e) => setForm({ ...form, nombre_tipo_indicador: e.target.value })} sx={{ mb: 2.5 }} inputRef={firstFieldRef} />
