...
const TipoInstitucionList = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('¿Eliminar este tipo de institución?')) return;
    try {
    ...
    <>
      <TableContainer component={Paper}>
        <Box sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6">Tipos de institución</Typography>
        ...
        {loading ? (
          ...
          <Table size="small">
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id_tipo_institucion}>
                  <TableCell>{row.nombre_tipo_institucion}</TableCell>
                  <TableCell>{row.descripcion_tipo_institucion}</TableCell>
                  <TableCell align="right">
                    <Button size="small" onClick={() => openEdit(row)} sx={{ mr: 0.5 }}>Editar</Button>
                    <Button size="small" color="error" onClick={() => handleDelete(row.id_tipo_institucion)}>Eliminar</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <Dialog open={openModal} onClose={() => setOpenModal(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingId ? 'Editar tipo de institución' : 'Nuevo tipo de institución'}</DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <TextField fullWidth label="Nombre" value={form.nombre_tipo_institucion} onChange={(e) => setForm({ ...form, nombre_tipo_institucion: e.target.value })} sx={{ mb: 2 }} />
