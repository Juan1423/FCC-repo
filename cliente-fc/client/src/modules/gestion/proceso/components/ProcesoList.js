...
const ProcesoList = () => {
  ...
  const fetchNormativas = async () => {
    try {
      ...
      const map = {};
      (res.data || []).forEach((n) => {
        map[n.id_normativa] = n.tipo_permiso || `Normativa ${n.id_normativa}`;
      });
  ...
  const handleEditModal = (item) => {
    ...
    setFormData({
      ...
      id_normativa: item.id_normativa,
    });
    setOpenModal(true);
  };
  ...
  const handleCreateSubmit = async () => {
    ...
    const payload = {
      ...formData,
      id_tipo_proceso: formData.id_tipo_proceso
        ? Number(formData.id_tipo_proceso)
        : null,
      id_normativa: formData.id_normativa
        ? Number(formData.id_normativa)
        : null,
    };
    try {
      if (editingId) {
      ...
      setError(
        editingId
          ? `Error al actualizar proceso: ${serverMessage}`
          : `Error al crear proceso: ${serverMessage}`,
      );
    }
  };
  ...
  return (
    <>
            ...
            <TableBody>
              {items.map((p) => (
                <TableRow key={p.id_proceso}>
                  ...
                  <TableCell>
                    {normativasMap[p.id_normativa] || "N/A"}
                  </TableCell>
                  <TableCell>
                    <Button
                      size="small"
      ...
      >
        <DialogTitle>
          {editingId ? "Editar Proceso" : "Crear Nuevo Proceso"}
        </DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <TextField
            ...
            onChange={handleChange("nombre_proceso")}
            sx={{ mb: 2 }}
          />
          <TextField
            fullWidth
            label="Descripción Proceso"
            value={formData.descripcion_proceso}
            onChange={handleChange("descripcion_proceso")}
            multiline
            rows={3}
            sx={{ mb: 2 }}
          />
          <TextField
            ...
            onChange={handleChange("entregable")}
            sx={{ mb: 2 }}
          />
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            ...
            <Select
              value={formData.id_normativa}
              label="Normativa"
              onChange={handleChange("id_normativa")}
            >
